...
public class TSequencielle extends AST {
	...
	public TSequencielle(AST e1,AST e2) {
		...
		for (AST ast : e1.children) {	
			...
			graph.addEdge(ID+ast.ID, ID, ast.ID, true).setAttribute("ui.label",e1.labels.get(ast.ID) );			
		}
		for (AST ast : e2.children) {
			...
			if(graph.getEdge(ID+ast.ID)==null)
				graph.addEdge(ID+ast.ID, ID, ast.ID, true).setAttribute("ui.label",e2.labels.get(ast.ID) );			
		}
